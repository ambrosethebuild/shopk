...
class VendorsBloc extends BaseBloc {
  ...
  //BehaviorSubjects
  BehaviorSubject<bool> _showOnlyAllVendors = BehaviorSubject.seeded(false);
  BehaviorSubject<List<Vendor>> _justAddedVendors = BehaviorSubject();
  BehaviorSubject<List<Vendor>> _popularVendors = BehaviorSubject();
  BehaviorSubject<List<Vendor>> _allVendors = BehaviorSubject();
  BehaviorSubject<List<Vendor>> _filteredVendors = BehaviorSubject();
  BehaviorSubject<DeliveryAddress> _selectedDeliveryAddress = BehaviorSubject();
  BehaviorSubject<int> _vendorListingStyle = BehaviorSubject.seeded(1);
  ...
  Stream<int> get vendorListingStyle => _vendorListingStyle.stream;
  ...
  void updatedDeliveryAddress(DeliveryAddress deliveryAddress) {
    _selectedDeliveryAddress.add(deliveryAddress);
  }

  void updatedListingStyle(int style) {
    _vendorListingStyle.add(style);
  }
}
